...
/**
 * All methods in this class are working with Androids toolbox. Toolbox is
 * similar to busybox, but normally shipped on every Android OS. You can find
 * toolbox commands on
 * https://github.com/CyanogenMod/android_system_core/tree/ics/toolbox
 * 
 * This means that these commands are designed to work on every Android OS, with
 * a _working_ toolbox binary on it. They don't require busybox!
 * 
 */
public class Toolbox {
	...
	/**
	 * All methods in this class are working with Androids toolbox. Toolbox is
	 * similar to busybox, but normally shipped on every Android OS.
	 * 
	 * @param shell
	 ...
	 * @throws IOException {@link com.perk.perksdk.adblocker.Toolbox}
	 * @throws TimeoutException {@link com.perk.perksdk.adblocker.Toolbox}
	 * @throws BrokenBusyboxException {@link com.perk.perksdk.adblocker.Toolbox}
	 */
	public boolean isRootAccessGiven() throws BrokenBusyboxException,
	 ...
	 * @throws BrokenBusyboxException {@link com.perk.perksdk.adblocker.Toolbox}
	 */
	public boolean killAll(String processName) throws BrokenBusyboxException,
			TimeoutException, IOException {
	 ...
	 * @throws IOException {@link com.perk.perksdk.adblocker.Toolbox}
	 */
	public boolean killAllExecutable(String executableName)
			throws BrokenBusyboxException, TimeoutException, IOException {
		return killAll(ExecutableCommand.EXECUTABLE_PREFIX + executableName
	 ...
	 * @throws IOException {@link com.perk.perksdk.adblocker.Toolbox}
	 */
	public boolean isBinaryRunning(String binaryName)
			throws BrokenBusyboxException, TimeoutException, IOException {
		return isProcessRunning(ExecutableCommand.EXECUTABLE_PREFIX
				+ binaryName + ExecutableCommand.EXECUTABLE_SUFFIX);
	}
	...
	private class LsCommand extends Command {
		...
		/**
		 * Converts permission string from ls command to numerical value.
		 * Example: -rwxrwxrwx gets to 777
		 * 
		 * @param permissions
		 * @return
		 */
			...
			if (line.contains(fileName)) {
				...
				try {
					...
					if (permissionMatcher.find()) {
						permissions = convertPermissions(permissionMatcher
								.group(1));

						Log.d(RootCommands.TAG, "Found permissions: "
								+ permissions);
					} else {
						Log.d(RootCommands.TAG,
								"Permissions were not found in ls command!");
					}
					...
					if (symlinkMatcher.find()) {
						/*
						 * TODO: If symlink points to a file in the same
						 * directory the path is not absolute!!!
						 */
	 ...
	 *            file and its name.
	 * @return File permissions as String, for example: 777, returns null on
	 *         error
	 * @throws IOException {@link com.perk.perksdk.adblocker.Toolbox}
	 * @throws TimeoutException {@link com.perk.perksdk.adblocker.Toolbox}
	 * @throws BrokenBusyboxException {@link com.perk.perksdk.adblocker.Toolbox}
	...
	public boolean setFilePermissions(String file, String permissions)
			throws BrokenBusyboxException, TimeoutException, IOException {
		Log.d(RootCommands.TAG, "Set permissions of " + file + " to "
				+ permissions);

		SimpleCommand chmodCommand = new SimpleCommand("chmod " + permissions
				+ " " + file);
	 ...
	 * 
	 * @param file
	 *            The path to the file to get the Symlink for. (must have
	 *            absolute path)
	 * 
	 * @return A String that represent the symlink for a specified file or null
	 *         if no symlink exists.
	 * @throws IOException {@link com.perk.perksdk.adblocker.Toolbox}
	 * @throws TimeoutException {@link com.perk.perksdk.adblocker.Toolbox}
	 * @throws BrokenBusyboxException {@link com.perk.perksdk.adblocker.Toolbox}
	 */
	public String getSymlink(String file) throws BrokenBusyboxException,
			TimeoutException, IOException {
	...
	public boolean copyFile(String source, String destination,
			boolean remountAsRw, boolean preservePermissions)
			throws BrokenBusyboxException, IOException, TimeoutException {

		/*
		 * dd can only copy files, but we can not check if the source is a file
		 * without invoking shell commands, because from Java we probably have
		 * no read access, thus we only check if they are ending with trailing
		 * slashes
		 */
	...
	/**
	 * Shutdown or reboot device. Possible actions are REBOOT_HOTREBOOT,
	 * REBOOT_REBOOT, REBOOT_SHUTDOWN, REBOOT_RECOVERY
	 * 
	 * @param action
	 * @throws IOException {@link com.perk.perksdk.adblocker.Toolbox}
	 * @throws TimeoutException {@link com.perk.perksdk.adblocker.Toolbox}
	 * @throws BrokenBusyboxException {@link com.perk.perksdk.adblocker.Toolbox}
	 */
	public void reboot(int action) throws BrokenBusyboxException,
			TimeoutException, IOException {
	 ...
	 * 
	 */
	public boolean fileExists(String file) throws BrokenBusyboxException,
			TimeoutException, IOException {
	...
	/**
	 * Execute user defined Java code while having temporary permissions on a
	 * file
	 * 
	 * @param file
	...
	public void withPermission(String file, String permission,
			WithPermissions withPermissions) throws BrokenBusyboxException,
			TimeoutException, IOException {
		String oldPermissions = getFilePermissions(file);

		// set permissions (If set to 666, then Dalvik VM can also write to that
		// file!)
	...
	/**
	 * Execute user defined Java code while having temporary write permissions
	 * on a file using chmod 666
	 * 
	 * @param file
	 ...
	 * @throws IOException {@link com.perk.perksdk.adblocker.Toolbox}
	 */
	public void withWritePermissions(String file,
			WithPermissions withWritePermissions)
			throws BrokenBusyboxException, TimeoutException, IOException {
	 ...
	 * @throws IOException {@link com.perk.perksdk.adblocker.Toolbox}
	 */
	public void setSystemClock(final long millis)
			throws BrokenBusyboxException, TimeoutException, IOException {
	 ...
	 * @throws IOException {@link com.perk.perksdk.adblocker.Toolbox}
	 */
	public void adjustSystemClock(final long offset)
			throws BrokenBusyboxException, TimeoutException, IOException {
		withWritePermissions("/dev/alarm", new WithPermissions() {
		...
		if (mounts != null) {
			for (Mount mount : mounts) {
				if (path.contains(mount.getMountPoint().getAbsolutePath())) {
					Log.d(RootCommands.TAG,
							(String) mount.getFlags().toArray()[0]);
	...
	public boolean hasEnoughSpaceOnPartition(String target, long size) {
		try {
			...
			Log.i(RootCommands.TAG, "Checking for enough space: Target: "
					+ target + ", directory: " + directory + " size: " + size
	 ...
	 * @throws BrokenBusyboxException {@link com.perk.perksdk.adblocker.Toolbox}
	 */
	public void toggleAdbDaemon(boolean toggle) throws BrokenBusyboxException,
			TimeoutException, IOException {
		SimpleCommand disableAdb = new SimpleCommand(
				"setprop persist.service.adb.enable 0", "stop adbd");
		SimpleCommand enableAdb = new SimpleCommand(
				"setprop persist.service.adb.enable 1", "stop adbd", "sleep 1",
				"start adbd");
