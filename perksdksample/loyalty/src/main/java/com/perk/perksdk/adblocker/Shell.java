...
public class Shell implements Closeable {
	...
	private static final String LD_LIBRARY_PATH = System
	 ...
	 * @throws IOException
	 */
	public static Shell startRootShell(ArrayList<String> customEnv,
			String baseDirectory) throws IOException, RootAccessDeniedException {
		...
		Shell shell = new Shell(UnblockUtils.getSuPath(), customEnv,
				baseDirectory);
	 ...
	 * @throws IOException
	 */
	public static Shell startShell(ArrayList<String> customEnv,
	...
	public static Shell startCustomShell(String shellPath,
			ArrayList<String> customEnv, String baseDirectory)
			throws IOException {
	...
	private Shell(String shell, ArrayList<String> customEnv,
			String baseDirectory) throws IOException, RootAccessDeniedException {
		Log.d(RootCommands.TAG, "Starting shell: " + shell);

		// start shell process!
		shellProcess = UnblockUtils.runWithEnv(shell, customEnv, baseDirectory);

		// StdErr is redirected to StdOut, defined in Command.getCommand()
		stdOutErr = new BufferedReader(new InputStreamReader(
	...
	/**
	 * Writes queued commands one after another into the opened shell. After an
	 * execution a token is written to seperate command output on read
	 * 
	 * @throws IOException
	 */
	...
	public Command add(Command command) throws IOException {
		...
		synchronized (commands) {
			...
			// set shell on the command object, to know where the command is
			// running on
