...
public abstract class Command {
	...
	public void processOutput(String line) {
		 ...
		 * 
		 * It is giving "Value too large for defined data type" on certain file
		 * operations (e.g. ls and chown) in certain directories (e.g.
		 * /data/data)
		 */
	...
	public void terminated(String reason) {
		setExitCode(-1);
		Log.d(RootCommands.TAG, "Command " + id
				+ " did not finish, because of " + reason);
	}

	/**
	 * Waits for this command to finish and forwards exitCode into
		...
		synchronized (this) {
			while (!finished) {
				try {
					...
					Log.e(RootCommands.TAG,
							"InterruptedException in waitForFinish()", e);
				}
