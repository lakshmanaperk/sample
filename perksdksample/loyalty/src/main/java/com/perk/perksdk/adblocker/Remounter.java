...
class Remounter {
	...
	protected boolean remount(String file, String mountType) {
		...
		Log.d(RootCommands.TAG, "Remounting "
			...
			try {
				SimpleCommand command = new SimpleCommand(
						...
						"toolbox mount -o remount,"
								...
								+ " "
								+ mountPoint.getMountPoint().getAbsolutePath(),
						"mount -o remount," + mountType.toLowerCase(Locale.US)
								+ " "
								+ mountPoint.getDevice().getAbsolutePath()
								+ " "
								+ mountPoint.getMountPoint().getAbsolutePath(),
		...
		if (mountPoint != null) {
			Log.d(RootCommands.TAG,
					mountPoint.getFlags() + " AND "
							+ mountType.toLowerCase(Locale.US));
			if (mountPoint.getFlags()
					.contains(mountType.toLowerCase(Locale.US))) {
	...
	protected static ArrayList<Mount> getMounts() throws Exception {
		...
		// copy /proc/mounts to tempfile. Directly reading it does not work on
		// 4.3
